function [aPu, aE, aW, aN, aS, b] = get_u_coeffs(nx,ny,u,v,dxc,dxe,dyc,dye,dt)
%dt=0.00001;
dx=1/(nx-1);
dy=1/(ny-1);
...
Fw(2:nx,:) = 0.5*(u(1:nx-1,:     ) + u(2:nx,:)     ) .*celldy;   Dw(2:nx,:) = repmat(dxc(1:nx-1) , 1, ny) .*celldy;
Fe(2:nx,:) = 0.5*(u(3:nx+1,:     ) + u(2:nx,:)     ) .*celldy;   De(2:nx,:) = repmat(dxc(2:nx) ,   1, ny) .*celldy;
Fs(2:nx,:) = 0.5*(v(1:nx-1,1:ny  ) + v(2:nx,1:ny)  ) .*celldx;   Ds(2:nx,2:ny) = repmat(dye', nx-1, 1).*celldx(:,2:ny);
Fn(2:nx,:) = 0.5*(v(1:nx-1,2:ny+1) + v(2:nx,2:ny+1)) .*celldx;   Dn(2:nx,1:ny-1) = repmat(dye', nx-1, 1).*celldx(:,1:ny-1);
